    ...
    if st.button("🔍 Process", disabled=not (file1 and file2)):
        try:
            ...
            if missing_cols_1 or missing_cols_2:
                ...
                st.info("Required columns: AWBIssuingAirline, AWBSerialNumber" + (", CargoWeight" if check_weight else ""))
            else:
                ...
                if len(duplicates_in_1) > 0:
                    ...
                    duplicate_report = create_duplicate_report(duplicates_in_1, duplicates_in_2, file1.name, file2.name, key_description)
